...
namespace ConsumerApp
{
    ...
    /// receive an ILoggerFactory instance. The Process method is called when a message is received, and it logs the content of the message along with its CorrelationId. The method returns a tuple indicating whether the message should be acknowledged and an optional reason for any failure.
    /// </summary>
    public class Consumer(ILoggerFactory loggerFactory) : IConsumer<Message>
    {
        /// <summary>
        /// The Consumer class is responsible for processing messages received from a RabbitMQ queue. It implements the IConsumer interface, which defines a method for processing messages. The Consumer class uses dependency injection to receive an IServiceScopeFactory and an ILoggerFactory instance. The Process method is called when a message is received, and it logs the content of the message along with its CorrelationId. The method returns a tuple indicating whether the message should be acknowledged and an optional reason for any failure.
        /// </summary>
        private readonly ILogger<Consumer> _logger = loggerFactory.CreateLogger<Consumer>();
        /// <summary>
        ...
        {
            _logger.LogInformation("[{CorrelationId}] Mensaje recibido: {Message}", message.CorrelationId, message.Data);
            return (true, null);
        }
    }
}